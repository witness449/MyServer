    ...
    : QObject()
{
    ...
    //this->requestHandler=new MyRequestMapper();
    ...
    //static_cast<MyRequestMapper*>(this->requestHandler)->ps=socket;
...
void HttpConnectionHandler::read()
{
    ...
    while (socket->bytesAvailable())
    {
               ...
               currentRequest->getStatus()!=HttpRequest::abort_broken)
        {
            ...
            //a=socket->bytesAvailable();
            if (currentRequest->getStatus()==HttpRequest::waitForBody)
            {
                //a=socket->bytesAvailable();
                // Restart timer for read timeout, otherwise it would
                // expire during large file uploads.
                int readTimeout=settings->value("readTimeout",100000).toInt();
                readTimer.start(readTimeout);
             }
        }
        ...
        else if (currentRequest->getStatus()==HttpRequest::waitForHeader)
        {
            qDebug("wait for header");
